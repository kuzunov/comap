...
const Profile = (props: Props) => {
  ...
  const {
    ...
    status,
    location,
  } = loaderData || currentUserState.currentUser;
  return (
    <Box>
      {currentUserState.isLoggedIn && !loaderData && (
        <Button
          variant="contained"
      ...
      <Avatar alt={username} src={avatar} sx={{ width: 100, height: 100 }} />
      <Typography variant="h2">
        {username}
        {gender === "m" ? <MaleIcon /> : <FemaleIcon />}{" "}
        {
          <LensIcon
            color={status === USER_STATUS.ACTIVE ? "success" : "error"}
          />
        }
      </Typography>
