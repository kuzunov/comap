...
export const UserContext = React.createContext<userContextType>({
  currentUserState: userInitialState,
  getToken: () => undefined,
});

export const UserContextProvider: React.FC<ProviderProps> = (
  props: ProviderProps
) => {
  const [cookies, setCookie, removeCookie] = useCookies(["comapauth"]);
  const loggedUser = cookies["comapauth"]
    ? cookies["comapauth"]
    : { userId: undefined };
  ...
  const [currentUserState, setCurrentUser] =
    useState<userState>(userInitialState);
  const setLoginPending = (isLoginPending: boolean) =>
    setCurrentUser({ ...currentUserState, isLoginPending });
  const setLoginSuccess = (isLoggedIn: boolean) =>
    setCurrentUser({ ...currentUserState, isLoggedIn });
  const setLoginError = (loginError?: string) =>
    setCurrentUser({ ...currentUserState, loginError });
  ...
  const login = async (user: Partial<IUser>) => {
    ...
    if (logged) {
      ...
      setLoginError(undefined);
      console.log(logged);
      if (logged.user.role === USER_ROLE.ADMIN) {
        setCurrentUser({ ...currentUserState, isAdmin: true });
      }
      setCurrentUser({
        ...currentUserState,
        isLoggedIn: true,
      ...
      setCookie(
        "comapauth",
        { token: logged.token, userId: logged.user.id },
