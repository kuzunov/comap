...
const EditUserForm = () => {
  ...
  const [serverErr, setServerErr] = useState<ServerError | undefined>(
    undefined
  );
  const {
    control,
  ...
  const validateAndReturnMarkers = () => {
    if (markers.length < 1)
      return { lat: 0, lng: 0 } as google.maps.LatLngLiteral;
  ...
  const onSubmit = async (
    ...
    if (!currentUserState.isLoggedIn) {
      try {
        ...
        if (login)
          login({ username: res.username, password: userToReg.password });
        navigate("/");
      } catch (err) {
  ...
  return (
    ...
    >
      ...
      <UserFormInputTextField
        ...
        type="password"
        disabled={
          getValues("id") !== currentUserState.currentUser.id &&
            ...
            <Controller
              ...
              render={({ field }) => (
                ...
                >
                  ...
                  <MenuItem value={USER_STATUS.DEACTIVATED}>
                    Deactivated
                  </MenuItem>
