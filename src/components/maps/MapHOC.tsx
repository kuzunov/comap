...
const MapHOC = ({center={lng:0,lat:0},markers,zoom=15,style={width:"500px", height:"300px"},editable,maxNumOfMarkers=20, updateMarkerArray}: MapHOCProps) => {
  ...
  const {
      ...
      const addMarker = (e:google.maps.MapMouseEvent) => {
       ...
       const changeMarkerPosition = (index:number,newPosition:google.maps.LatLngLiteral) => {
        if (newPosition!==mapMarkers[index].position){
          const newMarkersArray = [...mapMarkers];
          newMarkersArray.splice(index,1,{...mapMarkers[index],position:newPosition});
  ...
  return (
    ...
    <Wrapper apiKey={GOOGLE_MAPS_API_KEY}>
      ...
      renderInput={(params) => <TextField ref={controlRef} {...params} label="Search places" size="small" variant="filled" sx={{maxWidth:"60%"}}/>}/>
        ...
        >
        {mapMarkers.map((marker,index) => <Marker key={index} {...marker} 
                                              indexInArray={index} 
